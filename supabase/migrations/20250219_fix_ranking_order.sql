-- 랭킹 정렬 버그 수정
-- 문제: LEFT JOIN 후 ORDER BY가 없어 rank 순서가 보장되지 않음
-- 해결: 최종 SELECT에 ORDER BY r.rank 추가
...
-- 1. get_top_rankings (클래식 모드) - ORDER BY r.rank 추가
...
-- 2. get_time_attack_rankings (타임어택 모드) - ORDER BY r.rank 추가
